...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'HTS');
}
...
async function htsSearchRaw(keyword: string): Promise<HtsEntry[]> {
  const res = await pwFetch(`${HTS_BASE}/search?keyword=${encodeURIComponent(keyword)}`, {
    headers: { Accept: 'application/json', 'User-Agent': 'Pipeworx/1.0 (pipeworx.io)' },
  });
  if (!res.ok) throw await httpError(res, 'USITC HTS error');
